        ...
        <Stack spacing={2}>
          <Stack direction="row" alignItems="center" justifyContent="space-between">
            ...
            <Tooltip title={t('common.collapse')}>
              <IconButton size="small" onClick={onCollapse} sx={{ color: 'text.secondary' }} aria-label={t('common.collapse')}>
  ...
  return (
    <Stack direction="row" spacing={0.5} alignItems="center">
      <Tooltip title={t('common.previous')}>
        <IconButton size="small" onClick={nav.onPrev} disabled={nav.activeStep === 0} color="primary" aria-label={t('common.previous')}>
      ...
      <Tooltip title={t('common.next')}>
        <IconButton size="small" onClick={nav.onNext} disabled={nav.activeStep === nav.maxSteps - 1} color="primary" aria-label={t('common.next')}>
      ...
      >
        <Toolbar sx={{ gap: 1, minHeight: '52px !important' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1 }}>
            {!isDesktop ? (
              <IconButton size="small" color="inherit" onClick={() => setMobileOpen(true)} aria-label={t('common.openNavigation')}>
                <MenuRoundedIcon fontSize="small" />
              </IconButton>
            ) : collapsed ? (
              <Tooltip title={t('common.expand')}>
                <IconButton size="small" color="inherit" onClick={() => setCollapsed(false)} aria-label={t('common.expand')}>
                  <MenuRoundedIcon fontSize="small" />
                </IconButton>
              </Tooltip>
            ) : null}
